<template>
  <div class="trade-container">
    ...
    <div class="money clearFix">
      <ul>
        <li>
          ...
          <span>{{originalTotalAmount}}.00</span>
        </li>
        <li>
          <b>返现：</b>
          <span>0.00</span>
    ...
    <div class="sub clearFix">
      <a class="subBtn"  href="javascript:;"@click="hanlerSubmitPay">提交订单</a>
    </div>
  </div>
</template>
...
import {reqSubmitTradeOrder} from '@/api'
  export default {
    name: 'Trade',
    computed:{
       ...mapGetters('trade',['detailArrayList','originalTotalAmount','totalNum','totalAmount','tradeNo',]),
    },
    mounted(){
       this.$store.dispatch('trade/getUserAddressList')
      this.$store.dispatch('trade/getTradeInfoData')
    },
    methods:{
    async hanlerSubmitPay(){
        //准备数据
        const params={
          consignee:'张三',
          consigneeTel:'13410252516',
        deliveryAddress: "深圳市",
        //支付方式线上
        paymentWay: "ONLINE",
        //订单备注，这些在接口文档中有
        ...
        //发送请求
        const result=await reqSubmitTradeOrder(this.tradeNo,params)
        if(result.code===200){
         console.log(result)
         this.$message({
            type:'success',
