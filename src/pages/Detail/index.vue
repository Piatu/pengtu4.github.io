          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <!-- 判断点击的按钮是否被选中 -->
                <dd changepirce="0" :class="{'active':saleAttrValue.isChecked==='1'}" v-for="saleAttrValue in spuSaleAttr.spuSaleAttrValueList" :key="saleAttrValue.id" @click="handleActive(saleAttrValue,spuSaleAttr.spuSaleAttrValueList)">{{saleAttrValue.saleAttrValueName}}</dd>
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" v-model="count" class="itxt">
...
import { reqAddUpdateCart } from '@/api';
...
export default {
  ...
  methods: {
    ...mapActions('detail', ['getGoodDetailData']),
    //增加一个点击事件，让它刚开始的都不选中，然后点击某一个会选中，需要要给它传递参数让它知道点击的是那个
    //通过你点击的一瞬间获取saleAttrValue,spuSaleAttr.spuSaleAttrValueList从而知道获取的是哪个按钮
    //saleAttrValue就是被current接受，点击能获取到它的id，spuSaleAttr.spuSaleAttrValueList每一项按钮
    handleActive(current, parentAttr) {
      parentAttr.forEach(item => {
        //让所有spuSaleAttr.spuSaleAttrValueList下的按钮都为0未选中
        item.isChecked = '0';
      });
      //让当前点击的按钮选中
      current.isChecked = '1';
    ...
    // 2.加入购物车
//  派发action: 携带的载荷，分别商品的id、商品个数/ 思考底下的这行代码实质做了一个什么事情 ?
...
//  返回结果是一个Promise对象【三种状态: pending、成功、失败】，返回状态到底是什么，取决于这个函数addorupdateCart返回结果
    async add2Cart() {
      const result = await reqAddUpdateCart(this.id, this.count);
      if (result.code === 200) {
        //说明添加成功
        //跳转要给它要跳的路由
        console.log(112233);
  ...
  mounted() {
    // 发请求，并且出传递参数id
    this.getGoodDetailData(this.id);
  }
};
</script>
